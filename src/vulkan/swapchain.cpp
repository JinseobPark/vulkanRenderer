...
SwapChain::~SwapChain() {
    ...
    for (size_t i = 0; i < swapChainFramebuffers_.size(); i++) {
        vkDestroyFramebuffer(device_.device(), swapChainFramebuffers_[i], nullptr);
    }
...
VkResult SwapChain::submitCommandBuffers(const VkCommandBuffer* buffers, uint32_t* imageIndex) {
    ...
    // Use semaphore indexed by acquired image to avoid reuse conflicts
...
void SwapChain::createDepthResources() {
    ...
    for (size_t i = 0; i < depthImages_.size(); i++) {
        ...
        depthImageViews_[i] = device_.createImageView(
            depthImages_[i], 
            depthFormat_, 
...
void SwapChain::createSyncObjects() {
    ...
    renderFinishedSemaphores_.resize(imageCount());  // Per swapchain image
    ...
    for (size_t i = 0; i < imageCount(); i++) {
        if (vkCreateSemaphore(device_.device(), &semaphoreInfo, nullptr, &renderFinishedSemaphores_[i]) !=
                VK_SUCCESS) {
